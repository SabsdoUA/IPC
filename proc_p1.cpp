...
int main(int argc, char *argv[]) {
    FILE *out_log = std::fopen("proc_p1.out", "w");
    FILE *err_log = std::fopen("proc_p1.err", "w");
    ...
    if (!input.is_open()) {
        std::perror("proc_p1: fopen");
        if (err_log) std::fprintf(err_log, "proc_p1: cannot open %s\n", file_path);
        return 1;
    }
